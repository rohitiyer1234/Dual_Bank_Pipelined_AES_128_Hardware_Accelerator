`timescale 1ns/1ps
//=============================================================================
// tb_decrypt.sv  v3  - race-free, all 10 tests
//
// Ported 1:1 from tb_encrypt.sv v3. The DUT (AES_Decrypt) swaps the
// plaintext/ciphertext direction relative to AES_Encrypt: ciphertext is
// now the input driven by send_block, plaintext is the output checked
// against aes_ref_pkg::ref_decrypt_bank(). Every race fix below (RACE-1,
// RACE-2, RACE-3) and the negedge-only control-signal discipline applies
// identically here, since the handshake/pipeline timing is unchanged
// from AES_Encrypt - only the datapath transform differs.
//
// ====================================================================
...
always @(posedge clk) begin
    if (!reset && in_valid && in_ready) begin
        ...
        item.expected = aes_ref_pkg::ref_decrypt_bank(ciphertext, current_bank);
...
always @(posedge clk) begin
    if (!reset && out_valid && out_ready) begin
        if (expected_q.size() == 0) begin
            $error("[SB] Unexpected output  pt=%032h  t=%0t", plaintext, $time);
            fail_count++;
        end else begin
            sb_item_t item = expected_q.pop_front();
            if (plaintext === item.expected) begin
                pass_count++;
            end else begin
                fail_count++;
                $display("");
                ...
                $display("|  CT   = %032h", item.ct);
                $display("|  EXP  = %032h", item.expected);
                $display("|  DUT  = %032h", plaintext);
                $display("+==========================================+");
            end
        end
    end
...
//   PT  = 3243f6a8885a308d313198a2e0370734  (expected decrypt output)
//=============================================================================
...
task automatic test1_fips_kat();
    ...
    send_block(128'h3925841d02dc09fbdc118597196a0b32, 1'b0);
...
task automatic test6_const_pt_alt_bank(input int num_vectors = 100);
    int p0, f0;
    aes_block_t fixed_ct = 128'hdeadbeefcafebabedeadbeefcafebabe;
    $display("\n====================================");
    $display("TEST 6 : CONST CT / ALT BANK (%0d)", num_vectors);
    $display("====================================");
    $display("  CT     = %032h", fixed_ct);
    $display("  EXP_B0 = %032h", aes_ref_pkg::ref_decrypt_bank(fixed_ct, 1'b0));
    $display("  EXP_B1 = %032h", aes_ref_pkg::ref_decrypt_bank(fixed_ct, 1'b1));
    out_ready = 1;
    p0 = pass_count; f0 = fail_count;

    for (int i = 0; i < num_vectors; i++) begin
        logic bk = logic'(i[0]);
        send_block(fixed_ct, bk);
        @(posedge clk);
    end
    drain_scoreboard(num_vectors + 50);
...
task automatic test8_bank_valid_stress();
    ...
    @(negedge clk);
    ciphertext   = 128'hdeadbeefdeadbeefdeadbeefdeadbeef;
    current_bank = 1'b1;
    in_valid     = 1'b1;
    @(posedge clk);
    if (in_ready !== 1'b0)
    ...
    @(negedge clk);
    ciphertext   = 128'hcafebabeCAFEBABEcafebabeCAFEBABE;
    current_bank = 1'b0;
    in_valid     = 1'b1;
    @(posedge clk);
    if (in_ready !== 1'b0)
...
// P2: While stalled, plaintext (DUT output) must not change.
property p_stall_holds_pt;
    @(posedge clk) disable iff (reset)
    (out_valid && !out_ready) |=> (plaintext === $past(plaintext));
endproperty
assert property (p_stall_holds_pt)
    else $error("[SVA] plaintext changed during stall at %0t", $time);
